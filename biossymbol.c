...
void bfont_draw_large_letter(uint16 *buffer, uint8 c){
	int bufwidth = 640;
	uint8 *ch = (uint8*)bfont_find_symbol(c);
	uint16 word;
	buffer+=bufwidth;
	int x,y,z,j;
	x=0;y=0;
...
void bfont_draw_mid_symbol(uint16 *buffer, int chr){
	uint8   *ch = get_font_address()+chr;
	int bufwidth = 640;
    uint16  word;
    int x, y;
...
void bfont_draw_symbol(uint16 *buffer, char c){
	int bufwidth = 640;
	uint8 *ch = (uint8*)bfont_find_symbol(c);
	uint16 word;
	int x,y,z,j;
...
void bfont_draw_large_str(uint16 *buffer, int width, char *str) {
	for (str; *str != '\0'; str++){
